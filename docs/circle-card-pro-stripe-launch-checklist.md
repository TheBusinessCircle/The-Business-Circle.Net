...
7. Close the verification-link logging incident before accepting payment. After the safe logging/token release is running, an explicitly authorised database operator should invalidate all outstanding pre-release email-verification links without selecting or printing a token:
   ...
   The command reports a row count only; it makes every old verification link unusable, and affected users can request a new link generated by the fixed release. If the incident review confirms that password-reset or another authentication credential was also logged, revoke that credential class through its approved incident procedure before continuing. Do not delete or rotate unrelated invitation/referral codes speculatively. The payment window remains blocked until exposed links are invalidated or their enforced expiry has been independently confirmed.
