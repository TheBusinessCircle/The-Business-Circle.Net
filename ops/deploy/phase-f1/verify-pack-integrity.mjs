...
if (identity.forwardApplicationSha !== FORWARD_SHA || identity.rollbackApplicationSha !== ROLLBACK_SHA || identity.historicalProductionSha !== HISTORICAL_SHA || !/^[0-9a-f]{40}$/.test(identity.operationsCommit) || new Set([FORWARD_SHA, ROLLBACK_SHA, HISTORICAL_SHA, identity.operationsCommit]).size !== 4 || !/^[0-9a-f]{64}$/.test(identity.archiveSha256) || !/^[0-9a-f]{64}$/.test(identity.manifestSha256) || identity.candidateAggregate?.schemaVersion !== AGGREGATE_SCHEMA || !Number.isSafeInteger(identity.candidateAggregate.fileCount) || identity.candidateAggregate.fileCount < 1 || !/^[0-9a-f]{64}$/.test(identity.candidateAggregate.aggregateSha256 || "") || identity.installedPath !== `/opt/thebusinesscircle/deployment-packs/${identity.operationsCommit}`) throw new Error("Invalid external identity.");
