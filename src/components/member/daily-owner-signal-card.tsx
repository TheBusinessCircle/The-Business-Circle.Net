...
export function DailyOwnerSignalCard({ signal, isToday }: DailyOwnerSignalCardProps) {
  return (
    <article
      ...
      className={[
        "relative isolate flex min-h-[25rem] w-[86vw] shrink-0 snap-center flex-col justify-between overflow-hidden rounded-[1.6rem] border p-5 shadow-2xl transition-all duration-500 sm:w-[28rem] sm:p-6 lg:w-[31rem]",
        ...
        isToday
          ? "scale-[1.01] border-gold/42 text-foreground shadow-[0_24px_90px_rgba(212,175,55,0.18)]"
          : "border-silver/12 opacity-72 saturate-[0.82] shadow-black/30"
      ].join(" ")}
    >
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-gold/60 to-transparent" />
      <div className="absolute -right-16 -top-16 h-40 w-40 rounded-full bg-gold/10 blur-3xl" />
      <div className="absolute bottom-0 left-0 h-28 w-full bg-gradient-to-t from-black/36 to-transparent" />
      ...
      <div className="relative">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-[11px] uppercase tracking-[0.08em] text-silver">
            {formatSignalDate(signal.date)}
          </span>
        </div>

        <div className="mt-10 space-y-4">
          <p className="text-[11px] uppercase tracking-[0.12em] text-silver">
            {signal.seasonalTheme}
          </p>
          <h3 className="font-display text-3xl leading-tight text-foreground sm:text-4xl">
            {signal.title}
          </h3>
          <p className="max-w-[35rem] text-base leading-7 text-muted sm:text-[1.05rem]">
            {signal.bodyText}
          </p>
        </div>
      </div>
      ...
      <div className="relative mt-10 border-t border-white/10 pt-4">
        <p className="text-sm text-gold">{signal.footerMessage}</p>
      </div>
    </article>
  );
}
