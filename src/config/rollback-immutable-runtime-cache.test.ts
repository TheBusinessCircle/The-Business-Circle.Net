...
function copyMutableSyntheticFile(source: string, destination: string) {
  copyFileSync(source, destination);
  chmodSync(destination, 0o600);
}

function trustedProductionRuntimeNodeModules(source: string) {
  const nodeModules = join(source, "node_modules");
  const directory = lstatSync(nodeModules);
  if (
    realpathSync(nodeModules) !== nodeModules ||
    !directory.isDirectory() ||
    directory.isSymbolicLink() ||
    (process.platform !== "win32" &&
      (directory.uid !== 0 || directory.gid !== 0 || (directory.mode & 0o777) !== 0o555))
  ) {
    throw new Error("Production fixture runtime module root is not the trusted sealed directory.");
  }
  for (const relativePath of [
    ...
    if (
      ...
      file.nlink !== 1 ||
      (process.platform !== "win32" &&
        (file.uid !== 0 || file.gid !== 0 || (file.mode & 0o777) !== 0o444))
    ) {
      throw new Error("Production fixture Prisma runtime module is not trusted sealed material.");
    }
  }
  return nodeModules;
}
...
function strictFixtureProvenance(
  ...
  if (existsSync(join(canonicalSource, ".next", "cache"))) {
    throw new Error("Fixture contains forbidden build cache material.");
  }
...
function prepareProductionFixture(sourceRoot: string) {
  ...
  // Next may leave a complete build cache in the disposable build tree.
  // It is not an approved writable or persistent runtime artifact.
  ...
  symlinkSync(
    trustedProductionRuntimeNodeModules(source),
...
function createTemporaryCandidateRepository(options: {
  ...
  for (const file of ["package.json", "package-lock.json", "tsconfig.json"]) {
    copyMutableSyntheticFile(join(process.cwd(), file), join(root, file));
  }
  writeFileSync(
...
function createSyntheticFixture() {
  ...
  for (const file of [
    ...
    copyMutableSyntheticFile(join(fixtureCandidateRepository.root, file), join(source, file));
  }
...
function createGoogleFontMock(source: string) {
  ...
  const responses = {
    "https://fonts.googleapis.com/css2?family=Sora:wght@100..800&display=swap":
      "@font-face{font-family:'Sora';font-style:normal;font-weight:100 800;src:url(https://fonts.gstatic.com/s/sora/phase-e3-sora.woff2) format('woff2');}",
    "https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@200..800&display=swap":
      "@font-face{font-family:'Plus Jakarta Sans';font-style:normal;font-weight:200 800;src:url(https://fonts.gstatic.com/s/plusjakartasans/phase-e3-jakarta.woff2) format('woff2');}"
  };
...
function generateFreshProductionFixture(destination: string) {
  ...
  rmSync(fontMock);
  rmSync(join(target, ".next", "cache"), { recursive: true, force: true });
...
describe("rollback immutable runtime configuration and installed implementation guard", () => {
  it("normalizes sealed source modes only for mutable synthetic test copies", () => {
    const root = mkdtempSync(join(tmpdir(), "phase-e3-mutable-copy-"));
    temporaryDirectories.push(root);
    const source = join(root, "sealed-source.json");
    const destination = join(root, "mutable-copy.json");
    writeFileSync(source, '{"sealed":true}\n');
    chmodSync(source, 0o444);

    copyMutableSyntheticFile(source, destination);
    writeFileSync(destination, '{"mutatedForNegativeTest":true}\n');

    expect(readFileSync(source, "utf8")).toBe('{"sealed":true}\n');
    expect(readFileSync(destination, "utf8")).toBe('{"mutatedForNegativeTest":true}\n');
  });
...
describe("Phase E3 production fixture provenance", () => {
  ...
  it("rejects any sealed Next build cache material", () => {
    const source = createSyntheticFixture();
    rmSync(join(source, PROVENANCE_FILE));
    ...
    writeFixtureProvenance(source, buildFixtureProvenance(source, fixtureCandidateIdentity()));
    expect(() => validateSyntheticFixture(source)).toThrow(/forbidden build cache material/u);
  });
