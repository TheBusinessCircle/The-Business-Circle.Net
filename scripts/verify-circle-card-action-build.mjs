...
const forbiddenObjectReference = Object.values(manifest.node).find(
  (reference) => reference.exportedName === "CIRCLE_CARD_REPORT_IDLE_STATE"
);
if (forbiddenObjectReference) {
  throw new Error("The report idle-state object was emitted as a production server action.");
}

const references = Object.entries(manifest.node).filter(([, reference]) =>
  approvedExports.includes(reference.exportedName)
);
...
if (JSON.stringify(exportedNames) !== JSON.stringify([...approvedExports].sort())) {
  throw new Error(
    `Unexpected Circle Card production server-action exports: ${exportedNames.join(", ") || "none"}`
  );
}
